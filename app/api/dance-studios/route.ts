...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, logo, country, countryCode, city, order, website, googleMapsUrl } = body;
    ...
    const studio = await prisma.danceStudio.create({
      data: { name, logo: logo || "", country, countryCode, city: city || null, order: order ?? 0, website: website || null, googleMapsUrl: googleMapsUrl || null },
    });
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const body = await request.json();
    const { id, name, logo, country, countryCode, city, order, website, googleMapsUrl } = body;
    ...
    const studio = await prisma.danceStudio.update({
      where: { id },
      data: { name, logo, country, countryCode, city: city || null, order: order ?? 0, website: website || null, googleMapsUrl: googleMapsUrl || null },
    });
